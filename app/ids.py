...
def extract_features(packet) -> np.ndarray:
    ...
    Returns a NumPy array with features or None if not applicable.
    """
    if packet.haslayer(IP):
        ...
        # Reshape feature vector to 2D array as expected by the model
...
def detect_threat(packet, model) -> None:
    ...
    if features is not None:
        try:
            # Calculate score and prediction; adjust based on your model's API
        ...
        status = "🚨 Threat Detected!" if is_anomalous else "✅ Safe"
...
def start_detection(model, iface: str) -> None:
    ...
    # Use a lambda to pass the model to the detection function
